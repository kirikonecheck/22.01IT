...
### Описание команд Bash терминала

**clear** — очищает экран терминала.

**reset** — полностью сбрасывает терминал и очищает его буфер.

**history** — показывает историю ранее введённых команд.
...
**ll** — показывает подробный список файлов и папок (права доступа, размер, дата).

**ls --all** — показывает все файлы и папки, включая скрытые (начинающиеся с точки).

**lsb_release -a** — показывает информацию о версии операционной системы.

**neofetch** — показывает красивую информацию о системе (ОС, ядро, процессор, память).

**inxi -F** — показывает полную информацию о системе (аппаратное обеспечение, драйверы).

**w** — показывает, кто залогинен в системе и что они делают.

**id** — показывает информацию о текущем пользователе (UID, GID, группы).

**mkdir dir_name** — создаёт новую папку с указанным именем.

**mkdir folder{1..3}** — создаёт несколько папок с помощью раскрытия фигурных скобок.
